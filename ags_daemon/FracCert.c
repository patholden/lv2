/*
static char rcsid[] = "$Id: FracCert.c,v 1.2 2000/05/05 23:54:44 ags-sw Exp pickle $";
*/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
{
   struct parse_fraccert_resp *pRespBuf;
   ...
   int32_t   YrawAngle;
   int       status;
   uint32_t  respLen = (sizeof(struct parse_fraccert_resp) - kCRCSize);
...
#ifdef AGS_DEBUG
   syslog(LOG_DEBUG, "Entered Routine: FracCert");

   LogFracCertCommand(param, pLgMaster);
#endif

   pRespBuf = (struct parse_fraccert_resp *)pLgMaster->theResponseBuffer;
   ...
   status = DoubleSearchForASensor(pLgMaster, bXe, bYe, &dbXf, &dbYf);
...
#ifdef AGS_DEBUG
       LogFracCertResponse(pRespBuf, sizeof(pRespBuf->hdr.hdr));
#endif
       HandleResponse(pLgMaster, sizeof(pRespBuf->hdr.hdr), respondToWhom);
       return;
     }

   ConvertDoubleBinaryToMirrorAngles(dbXf, dbYf, &aXf, &aYf);
   ...
   syslog(LOG_NOTICE, "FracCert time diff %lf  end/start %lf %lf", time_diff, time_now, time_start);
   ...
   if (status)
   ...
   else
     {
       ...
       pRespBuf->dbXf = dbXf;
       pRespBuf->dbYf = dbYf;
       ConvertBinaryToExternalAngles(pLgMaster, dbXf, dbYf, &Xexternal, &Yexternal);
       pRespBuf->Xexternal = Xexternal;
       pRespBuf->Yexternal = Yexternal;
#ifdef AGS_DEBUG
       LogFracCertResponse(pRespBuf, respLen);
#endif
...
void LogFracCertCommand(struct parse_fraccert_parms *param, struct lg_master *pLgMaster)
{
    ...
    syslog(LOG_DEBUG, "CMD: inp_XrawAngle: %lf", param->inp_XrawAngle);

    syslog(LOG_DEBUG, "CMD: inp_YrawAngle: %lf", param->inp_YrawAngle);
...
{    
  syslog(LOG_DEBUG, "RSP: hdr: %08x",htonl(pRespBuf->hdr.hdr));
    ...
    syslog(LOG_DEBUG, "RSP: dbXf: %lf", pRespBuf->dbXf);

    syslog(LOG_DEBUG, "RSP: dbYf: %lf", pRespBuf->dbYf);
