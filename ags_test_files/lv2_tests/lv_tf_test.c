...
int main( int argc, char ** argv )
{
    ...
    usleep(1000);
    error = FindTarget(pConfigMaster, inputX, inputY, &foundX, &foundY);
    if (error)
      {
    ...
    if ((foundX == 0xFFFF) &&  (foundY == 0xFFFF))
      {
	syslog(LOG_NOTICE, "LV_TF_TEST: TargetFind target NOT FOUND");
	return(-1);
      }
    syslog(LOG_NOTICE, "LV_TF_TEST: Found target at %x,%x", foundX, foundY);
    ...
    syslog(LOG_NOTICE,"LV_TF_TEST: TargetFind Test complete for /dev/lv2, fd %d",pConfigMaster->fd_lv2);
    exit(EXIT_SUCCESS);
}
